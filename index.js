...
(function () {
  ...
  // Finding this extension's own folder URL turned out to need more
  // than one strategy. document.currentScript only works while a
  // classic, non-module <script> is synchronously first executing —
  // if SillyTavern loads extensions via dynamic import() instead (or
  // inserts the script in a way that detaches currentScript before
  // this line runs), it comes back null and every path built from it
  // is null too. That's silent: no error, the button/panel still work
  // fine since they don't need any file URL, so this went unnoticed
  // until sound was added and nothing played, on either the open chime
  // or the background music, at once — a symptom that only makes
  // sense if EXT_BASE_URL was never resolving in the first place.
  //
  // Rather than assume any one detection method is the one that'll
  // work in a given SillyTavern build, this tries three, in order of
  // reliability, and — since none of them are 100% guaranteed even
  // when they find *a* URL — every audio file is loaded with an
  // on-error fallback (see loadAudioWithFallback below) that advances
  // to the next candidate automatically if the first one turns out to
  // be wrong.
  function detectExtBaseUrlCandidates() {
    const candidates = [];

    // Strategy 1: document.currentScript, when available.
    try {
      const src = document.currentScript && document.currentScript.src;
      if (src) candidates.push(src.replace(/index\.js(\?.*)?$/, ""));
    } catch (e) {
      /* ignore */
    }

    // Strategy 2: scan every <script src> already in the page for one
    // that looks like this extension's own index.js — matches on
    // "/extensions/" plus a case-insensitive "obsession" somewhere in
    // the path, so it doesn't depend on the exact folder name someone
    // installed it under.
    try {
      document.querySelectorAll("script[src]").forEach((s) => {
        const src = s.src || "";
        if (/\/extensions\//i.test(src) && /obsession/i.test(src) && /index\.js(\?.*)?$/i.test(src)) {
          candidates.push(src.replace(/index\.js(\?.*)?$/, ""));
        }
      });
    } catch (e) {
      /* ignore */
    ...
    // Strategy 3: known SillyTavern convention for a third-party
    // extension installed by folder name — confirmed against this
    // extension's own server startup log, which reports it as
    // `{ type: 'local', name: 'third-party/obsessiontracker' }`.
    // Included as both common folder-name spellings in case the
    // install folder is named differently than expected.
    candidates.push("/scripts/extensions/third-party/obsessiontracker/");
    candidates.push("/scripts/extensions/third-party/obsession-tracker/");

    // De-duplicate while preserving order.
    return candidates.filter((c, i) => c && candidates.indexOf(c) === i);
  }

  const EXT_BASE_URL_CANDIDATES = detectExtBaseUrlCandidates();

  // Creates an Audio element pointed at `filename` under the first
  // candidate base URL, and if it fails to load (404, wrong path,
  // etc.), automatically retries under the next candidate, and so on
  // — instead of committing to one guess and staying silently broken
  // if that guess was wrong. Logs which candidate actually worked
  // once one does, so a future "no sound" report has something
  // concrete to check first.
  function loadAudioWithFallback(filename, label) {
    if (!EXT_BASE_URL_CANDIDATES.length) {
      console.error(`[ObsessionMeter] No base URL candidates found for ${label} (${filename}) — sound disabled.`);
      return null;
    }
    const audio = new Audio();
    let idx = 0;
    function tryNext() {
      if (idx >= EXT_BASE_URL_CANDIDATES.length) {
        console.error(
          `[ObsessionMeter] ${label} (${filename}) failed to load from every candidate URL tried: ` +
            EXT_BASE_URL_CANDIDATES.map((c) => c + filename).join(", ")
        );
        return;
      }
      const url = EXT_BASE_URL_CANDIDATES[idx] + filename;
      idx++;
      audio.src = url;
    }
    ...
    audio.addEventListener(
      "canplaythrough",
      () => {
        console.log(`[ObsessionMeter] ${label} loaded successfully from: ${audio.src}`);
      },
      { once: true }
    );
    tryNext();
  ...
  // Panel-open sound. Created once and reused (rather than a `new
  // Audio()` per open) so repeated opens don't leak audio elements.
  // preload:"auto" starts fetching it immediately rather than waiting
  // for the first play() call, so the first open doesn't have to eat
  // the load latency too.
  const openSound = loadAudioWithFallback("open.mp3", "open chime");
  if (openSound) openSound.preload = "auto";

  function playOpenSound() {
    if (!openSound) return;
    try {
      if (!getGlobalSettings().musicEnabled) return;
    } catch (e) {
      /* context not ready yet — default to playing */
    }
    try {
      openSound.currentTime = 0;
      const p = openSound.play();
      // play() returns a Promise that rejects if the browser's
      // autoplay policy blocks it (shouldn't happen here since this
      // only ever fires from a real click, but browsers vary) — catch
      // it so a rejection can't surface as an unhandled promise
      // rejection or, worse, throw out of the click handler and break
      // the panel opening itself.
      if (p && typeof p.catch === "function") {
        p.catch((e) => console.warn("[ObsessionMeter] open sound couldn't play:", e));
      }
    } catch (e) {
      console.warn("[ObsessionMeter] open sound play() threw:", e);
    }
  }

  // ---------------------------------------------------------------
  // Background music — "Schoolday" plays on loop while sanity is
  // normal; it crossfades (pause/play swap — see switchBgTrack) to
  // the low-sanity track once the lowest sanity across all tracked
  // characters in the current chat drops low enough, and swaps back
  // once it recovers.
  //
  // Hysteresis (enter at 30, exit at 40) is deliberate: a single
  // threshold right at the boundary would flip tracks back and forth
  // on every small stat tick if sanity happened to hover near it
  // (e.g. decay nudging it up 1 point, a message nudging it back
  // down) — audibly restarting the track each time. A gap between the
  // enter/exit points means it has to cross a real margin either way
  // before switching again.
  // ---------------------------------------------------------------

  const bgNormalSound = loadAudioWithFallback("bg-normal.mp3", "background music (normal)");
  const bgLowSanitySound = loadAudioWithFallback("bg-low-sanity.mp3", "background music (low sanity)");
  [bgNormalSound, bgLowSanitySound].forEach((a) => {
    if (!a) return;
    a.loop = true;
    a.preload = "auto";
    // Kept a bit under full volume — this plays continuously in the
    // background rather than as a one-shot cue like openSound, so it
    // shouldn't compete with ST's own UI sounds or a TTS voice.
    a.volume = 0.5;
  });

  const SANITY_LOW_ENTER = 30; // drop to/below this -> switch to low-sanity track
  const SANITY_LOW_EXIT = 40; // rise to/above this -> switch back to normal track

  let currentBgTrack = null; // "normal" | "low" | null (nothing decided/started yet)
  let bgMusicStarted = false; // true once a real user gesture has unlocked audio playback

  // Lowest sanity among every character tracked in the *current chat*
  // (not just whichever one the panel happens to be showing) — so the
  // music reacts to the state of the story, not to which character
  // page you're idly viewing. Defaults to full sanity (i.e. "normal"
  // track) if nothing's been tracked yet.
  function getLowestSanity() {
    try {
      const d = getChatData();
      let lowest = null;
      for (const name of Object.keys(d.characters || {})) {
        const v = d.characters[name] && d.characters[name].values ? d.characters[name].values[SANITY_KEY] : undefined;
        if (typeof v === "number" && (lowest === null || v < lowest)) lowest = v;
      }
      return lowest === null ? SANITY_MAX : lowest;
    } catch (e) {
      console.warn("[ObsessionMeter] getLowestSanity() failed, defaulting to normal:", e);
      return SANITY_MAX;
    }
  }

  // Plays or pauses whichever track is `currentBgTrack` to match
  // whether the panel is actually open right now, without touching
  // playback position — this is what makes closing the panel pause
  // the music instead of leaving it running underneath, and reopening
  // it resume from where it left off rather than restarting the
  // track. Also makes sure the *other* track (the one not currently
  // selected) is always paused, so a mid-switch never leaves both
  // tracks running at once.
  function applyBgPlaybackState() {
    const active = currentBgTrack === "low" ? bgLowSanitySound : currentBgTrack === "normal" ? bgNormalSound : null;
    const inactive = currentBgTrack === "low" ? bgNormalSound : bgLowSanitySound;
    if (inactive) {
      try {
        inactive.pause();
      } catch (e) {
    ...
    try {
      musicEnabled = getGlobalSettings().musicEnabled;
    } catch (e) {
      /* context not ready yet — default to enabled */
    }
    if (panelOpen && bgMusicStarted && musicEnabled) {
      try {
        const p = active.play();
        if (p && typeof p.catch === "function") {
          p.catch((e) => console.warn("[ObsessionMeter] background music couldn't play:", e));
        }
      } catch (e) {
        console.warn("[ObsessionMeter] background music play() threw:", e);
      }
    } else {
      try {
        active.pause();
  ...
  function switchBgTrack(target) {
    if (currentBgTrack === target) return;
    currentBgTrack = target;
    // Reset position only on an actual track change, not on every
    // panel open/close — applyBgPlaybackState() below handles the
    // play/pause without touching currentTime, so resuming an
    // already-current track picks up where it left off.
    const incoming = target === "low" ? bgLowSanitySound : bgNormalSound;
    if (incoming) {
      try {
    ...
    applyBgPlaybackState();
  }

  // Re-evaluates which track *should* be playing right now and swaps
  // if the hysteresis band has been crossed. Called after anything
  // that can change sanity (a [STATS:] tag, decay) and after a chat
  // switch (a different chat can have wildly different sanity state).
  function updateBackgroundMusic() {
    if (!bgNormalSound && !bgLowSanitySound) return;
    const lowest = getLowestSanity();
    const wantsLow = currentBgTrack === "low" ? lowest < SANITY_LOW_EXIT : lowest <= SANITY_LOW_ENTER;
    const target = wantsLow ? "low" : "normal";
    if (target === currentBgTrack) {
      // Track itself isn't changing, but panel open/closed state might
      // have — make sure playback still matches (e.g. sanity ticked
      // while the panel was closed; nothing should start playing until
      // it's reopened).
      applyBgPlaybackState();
    } else {
      switchBgTrack(target);
    }
  }

  // Browsers block audio from playing before the page has seen a real
  // user gesture (tap/click) — a plain autoplay-on-load call would
  // just be silently rejected. So instead: decide the correct track
  // immediately (currentBgTrack gets set right away), but only issue
  // the actual play() call the moment the very first tap/click
  // anywhere on the page happens — which for a phone is normally the
  // instant someone starts using the tab at all. Playback still only
  // actually starts once the panel is open (see applyBgPlaybackState).
  ...
  //
  // The body half is deliberately optional here (was previously
  // required, `\s*([^\n\[]+)`) to also catch a common model slip:
  // writing the whole thing inside one bracket —
  // `[STATS:Aiden love+5, trust-2]` instead of the documented
  // `[STATS:Aiden] love+5, trust-2`. With a required body, that
  // malformed form matched nothing at all: no error, nothing in the
  // log, the tag just vanished, which is exactly why it can look like
  // "the tracker isn't tracking" with nothing to point at. Making the
  // body optional lets it match, and splitMergedStatsTag() below
  // recovers the name/stats split from inside that single bracket.
  const STATS_TAG = /\[STATS:([^\]]+)\](?:\s*([^\n\[]+))?/g;

  // Recovers a name + stats body from a merged single-bracket STATS
  // tag (see STATS_TAG comment above). Finds the first change token
  // (a `+`, `-`, or `=` immediately followed by a number) and walks
  // left from it to the start of that one contiguous word — stat keys
  // in every documented example are single words (love, trust,
  // possession, sanity, obsession...), so this deliberately doesn't
  // cross a space, which keeps a genuinely multi-word character name
  // ("Aiden Cross") intact rather than eating part of it as a key.
  // Returns null if no recognizable key+change pattern is found at
  // all, so a truly malformed tag still gets skipped rather than
  // guessed at.
  function splitMergedStatsTag(raw) {
    const opMatch = raw.match(/(\+|-|=)\s*-?\d/);
    if (!opMatch) return null;
    let start = opMatch.index;
    while (start > 0 && /[a-zA-Z0-9_]/.test(raw[start - 1])) start--;
    const name = raw.slice(0, start).trim();
  ...
  function getGlobalSettings() {
    ...
    if (typeof s.musicEnabled !== "boolean") s.musicEnabled = true;
    return s;
  }
  ...
  function applyInventoryChanges(name, actions, opts) {
    ...
    for (const action of actions) {
      if (action.op === "add") {
        ...
        changed = true;
        // Skipped during a silent bulk rescan — replaying a whole chat's
        // worth of [INVENTORY:...] tags shouldn't fire off an image-gen
        // request per historical item (slow, wasteful, and pointless for
        // any item a later message goes on to remove anyway). The regen
        // button on an item's detail view still works afterward.
        if (!silent) generateItemImage(item); // fire-and-forget; re-renders/saves when settled
  ...
  // `opts.silent` skips the flash animation / per-tag re-render and
  // skips inventory image-gen — used by rescanChatHistory() when
  // replaying a whole chat's worth of messages in one pass, so a bulk
  // rescan doesn't flood the panel with 1.4s flash timeouts or fire a
  // slash command per historical item. Live per-message handling
  // (wireEvents' handler) always calls this without opts, unchanged.
  // Returns how many of each tag type were actually applied, so
  // callers (the rescanner) can report a useful count.
  function handleIncomingMessage(rawText, opts) {
    const counts = { stats: 0, diary: 0, inventory: 0 };
    if (!rawText) return counts;
    const silent = !!(opts && opts.silent);
    let sawSomething = false;
    for (const match of rawText.matchAll(STATS_TAG)) {
      const [, rawInner, maybeBody] = match;
      let name, body;
      if (maybeBody && maybeBody.trim()) {
        // Normal, documented form: [STATS:Name] stats-go-here
        name = rawInner.trim();
        body = maybeBody;
      } else {
        // Merged single-bracket form: [STATS:Name stats-go-here]
        const split = splitMergedStatsTag(rawInner);
        if (!split) {
          console.warn(`[ObsessionMeter] Couldn't recover a name/stats split from: "${rawInner}"`);
          continue;
        }
        name = split.name;
        body = split.body;
    ...
    for (const match of rawText.matchAll(INVENTORY_TAG)) {
      ...
      const changed = applyInventoryChanges(name, actions, { silent });
      if (changed) {
        sawSomething = true;
        counts.inventory++;
      }
    }
    if (sawSomething && !silent) {
      renderPanel();
      updateToggleGlow();
    }
    return counts;
  ...
  // ---------------------------------------------------------------
  // Rescan / resync — recovers from a message whose tag was missed
  // live (streaming race, a dedupe false-positive, the extension
  // having failed to load yet when the message arrived, etc).
  //
  // This is a full rebuild, not an incremental "catch up": it wipes
  // this chat's stored characters/diary/inventory/log and replays
  // every message in the chat, in order, through the exact same
  // parsing/apply path a live message goes through. That's the only
  // way to be sure of the result — there's no reliable record of
  // which past messages' tags were already counted vs silently
  // dropped, so incrementally re-scanning "just the missed ones"
  // risks double-applying a `love+5` that actually landed the first
  // time. A full replay is deterministic: run it twice in a row and
  // you get the same numbers both times.
  //
  // Trade-off worth knowing: anything set by hand (manual stat edits,
  // a hand-typed diary entry, a manually-added inventory item — all
  // supported per the README) has no tag backing it, so a rebuild
  // discards it. The caller is expected to confirm with the user
  // before calling this.
  function rescanChatHistory() {
    const fresh = getFreshContext();
    ...
    const totals = { messages: 0, stats: 0, diary: 0, inventory: 0 };
    for (const msg of chatArr) {
      const text = msg && typeof msg.mes === "string" ? msg.mes : "";
      if (!text) continue;
      totals.messages++;
      const counts = handleIncomingMessage(text, { silent: true });
      totals.stats += counts.stats;
      totals.diary += counts.diary;
      totals.inventory += counts.inventory;
    }

    saveChatData();
    // Bulk pass ran silent (no per-tag re-render) — repaint once now
    // that it's done, same as the live handler does per-message.
    renderPanel();
    updateToggleGlow();
    return totals;
  }
  ...
  // reach it from.
  // Persisted only for the current session (not saved to disk) so the
  // result of the last rescan stays visible across the re-render that
  // onChange() triggers, instead of vanishing the instant it's shown.
  ...
  function renderSettingsFieldsHtml() {
    ...
    return `
      ...
      <div class="om-settings-section">
        <label class="om-checkbox-label">
          <input type="checkbox" class="om-music-toggle" ${s.musicEnabled ? "checked" : ""} />
          Background music (Schoolday / low-sanity track)
        </label>
        <p class="om-imagegen-help">Also covers the panel-open chime. Turning
        this off pauses whatever's currently playing immediately; turning it
        back on resumes on the next panel open.</p>
      </div>
      <div class="om-settings-section">
        <label class="om-checkbox-label">
      ...
      <div class="om-settings-section">
        ...
        <p class="om-imagegen-help">If a character's reply had a tag that
        never got picked up (e.g. it arrived while the extension was still
        loading, or during a streaming reply), this rebuilds every tracked
        value for the current chat by replaying every message in it from
        scratch — the same as if you were reading through from the start.
        <strong>It overwrites current values</strong>, so anything you set by
        hand (a manual stat edit, a hand-typed diary entry, a manually-added
        item) that isn't backed by an actual tag in the chat will be lost.
        Diary/inventory items placed by tags are unaffected.</p>
        <button class="om-rescan-btn menu_button">Rescan this chat</button>
        <div class="om-rescan-result">${lastRescanSummary ? escapeHtml(lastRescanSummary) : ""}</div>
      </div>
  ...
  function attachSettingsFieldHandlers(root, onChange) {
    ...
    const rescanBtn = root.querySelector(".om-rescan-btn");
    if (rescanBtn) {
      rescanBtn.addEventListener("click", () => {
        const ok = window.confirm(
          "Rebuild this chat's stats/diary/inventory from scratch by replaying " +
            "every message? This overwrites current values with whatever the " +
            "tags in the chat actually say — any hand-edited values not backed " +
            "by a tag will be lost. This can't be undone."
        );
        if (!ok) return;
        rescanBtn.disabled = true;
        rescanBtn.textContent = "Rescanning…";
        // Let the disabled/label update paint before the (synchronous,
        // potentially chat-length-proportional) rebuild runs.
        setTimeout(() => {
          const totals = rescanChatHistory();
          lastRescanSummary = totals
            ? `Scanned ${totals.messages} messages — applied ${totals.stats} stat tag(s), ` +
              `${totals.diary} diary entr${totals.diary === 1 ? "y" : "ies"}, ` +
              `${totals.inventory} inventory tag(s).`
            : "Couldn't read this chat's messages — no chat metadata store available.";
          // onChange() re-renders renderSettingsFieldsHtml() fresh, which
          // is why the summary lives in lastRescanSummary rather than
          // being written directly into rescanResult's textContent here —
          // a direct DOM write would just get wiped by that re-render.
          onChange();
        }, 10);
      });
    }
    ...
    const musicToggle = root.querySelector(".om-music-toggle");
    if (musicToggle) {
      musicToggle.addEventListener("change", () => {
        const s = getGlobalSettings();
        s.musicEnabled = musicToggle.checked;
        saveGlobalSettings();
        // Apply immediately — pauses right away if turned off, resumes
        // the current track right away if turned back on (rather than
        // waiting for the next panel open/close or sanity change).
        applyBgPlaybackState();
      });
    }
  }
  ...
  async function init() {
    ...
    // Decide + arm background music immediately. The actual play()
    // call is deferred until the first real tap/click (browser
    // autoplay policy — see startBackgroundMusicOnFirstInteraction),
    // but this is unconditional/best-effort like the UI build below,
    // so it's wired up before anything that could throw gets a chance
    // to skip it.
    try {
      updateBackgroundMusic();
      startBackgroundMusicOnFirstInteraction();
    } catch (e) {
      console.warn("[ObsessionMeter] background music setup failed:", e);
    }
